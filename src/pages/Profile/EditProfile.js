...
export default function EditProfile({ navigation }) {
  ...
  const deleteCurrentAvatarFromStorage = async () => {
    if (userData.avt && userData.avt !== avtImg && userData.avt.includes("userAvatars")) {
      const storageRef = storage().refFromURL(userData.avt)
      const imageRef = storage().ref(storageRef.fullPath)
      imageRef
        .delete()
        .then(() => console.log('avt deleted'))
        .catch(e => console.log('Err in delete img', e, userData.avt))
    }
  }

  const handleUploadImage = async () => {
    let imgUrl = null
    if (avtImg && avtImg !== userData.avt) {
      ...
      >
        ...
        <View>
          <View>
            ...
            <TextInput style={styles.textInput} label="Phone Number"
              keyboardType="number-pad"
              theme={{
                colors: {
                  text: "black",
